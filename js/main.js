...
function displayProducts(defaultDisplayOrSearch) {
   ...
   products.forEach((item, index) => {
      ...
      if (checkSearch) {
         ...
         <td>${item.category}</td>
         <td>${item.descrip}</td>
         <td> <i class="fa-solid fa-pen-to-square me-1 text-secondary icon-table" id="btnEdit" onclick="getUpdateInfo(${index})"></i></td>
         <td><i class="fa-solid fa-trash-can text-danger icon-table" id="btnDelete" onclick="deleteRow(${index})"></i></td>
      </tr>
         `;
      }
   });
